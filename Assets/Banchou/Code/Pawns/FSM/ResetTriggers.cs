...
namespace Banchou.Pawn.FSM {
    public class ResetTriggers : FSMBehaviour {
        ...
        [SerializeField, Tooltip("If resetting at state time, what time")]
        private float _atStateTime;

        [SerializeField, Tooltip("If resetting at time after state entry, what time")]
        ...
        public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex) {
            if (_onEvent.HasFlag(ApplyEvent.OnEnter)) {
                foreach (var hash in _hashes) animator.ResetTrigger(hash);
            }
            _time = 0f;
        }
